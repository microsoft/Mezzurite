...
awa.cv = (function()
{
    ...
    return {
            header: header, tag: eventTag, isInit: isInit, canExtend: canExtend, canIncrement: canIncrement, getValue: getValue, setValue: setValue, init: init, extend: extend, increment: increment, incrementExternal: incrementExternal, isValid: isValid, useCv1: useCv1, useCv2: useCv2
        }
})();
awa.logger = (function()
{
    ...
    var populatePartAandExtensions = function(event)
        {
            ...
            event.ver = "2.1";
            event.cV = isValueAssigned(cV) ? cV : awa.cv.getValue();
            ...
            var javascriptExtensionObject = {
                    ver: "1.1", libVer: "4.2.4", domain: domain, msfpc: awa.cookie.getCookie("MSFPC", "GUID"), userConsent: isValueAssigned(config.userConsented) ? config.userConsented : (isValueAssigned(awa.cookie.getCookie(config.userConsentCookieName)) ? true : false)
                };
            if (event.ext)
            {
    ...
    function sendPageView(overrideValues)
    {
        ...
        var pageViewEvent = {
                name: "Ms.Webi.PageView", data: {
                        ...
                        }, baseType: "Ms.Content.PageView", cookieEnabled: getCookieEnabled(), cookies: getClientCookies(), isJs: true, title: utils.getTitle(), isLoggedIn: config.isLoggedIn, serverImpressionGuid: eventData.serverImpressionGuid, isManual: true
    ...
    function sendContentUpdate(overrideValues)
    {
        ...
        var contentUpdateEvent = {
                name: "Ms.Webi.ContentUpdate", data: {
                        ...
                        }, baseType: "Ms.Content.ContentUpdate", timings: JSON.stringify(overrideValues.timings), title: utils.getTitle(), cookieEnabled: getCookieEnabled(), isJs: true, isManual: true, isDomComplete: false, isLoggedIn: config.isLoggedIn, serverImpressionGuid: eventData.serverImpressionGuid
                    }, flags: eventData.isSensitive ? IS_SENSITIVE_FLAG : undefined
            };
